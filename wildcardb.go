...
func MatchSimpleB(pattern, name []byte) bool {
	lp := len(pattern)
	if lp == 0 {
		return len(name) == 0
	}
	...
	return deepMatchRuneB(name, pattern, true)
}
...
func MatchB(pattern, name []byte) (matched bool) {
	lp := len(pattern)
	if lp == 0 {
		return len(name) == 0
	}
	...
	// Do an extended wildcard '*' and '?' match.
	return deepMatchRuneB(name, pattern, false)
}

func deepMatchRuneB(str, pattern []byte, simple bool) bool {
	for len(pattern) > 0 {
		switch pattern[0] {
		default:
		...
		case '*':
			return len(pattern) == 1 || // Pattern ends with this star
				deepMatchRuneB(str, pattern[1:], simple) || // Matches next part of pattern
				(len(str) > 0 && deepMatchRuneB(str[1:], pattern, simple)) // Continue searching forward
		}
		str = str[1:]
		pattern = pattern[1:]
	}
...
func MatchAsPatternPrefixB(pattern, text []byte) bool {
